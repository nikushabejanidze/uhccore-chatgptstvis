...
public class PlayerConnectionListener implements Listener {
	...
	public void onPlayerJoin(final PlayerJoinEvent event) {
		Bukkit.getScheduler().runTaskLater(UhcCore.getPlugin(), () -> {
			playerManager.playerJoinsTheGame(event.getPlayer());

			// ✅ apply tab after join (ensures header/footer set)
			UhcCore.getPlugin().getTablistManager().apply(event.getPlayer());
		}, 1L);
	}

	// ✅ NEW: re-apply tab header/footer when changing world
	...
	public void onPlayerDisconnect(PlayerQuitEvent event) {
		...
		if (gameManager.getGameState().equals(GameState.PLAYING) || gameManager.getGameState().equals(GameState.DEATHMATCH)) {
			...
			if (gameManager.getConfig().get(MainConfig.ENABLE_KILL_DISCONNECTED_PLAYERS)
				...
				Bukkit.getScheduler().runTaskLater(UhcCore.getPlugin(), killDisconnectedPlayerTask, 1L);
			}
