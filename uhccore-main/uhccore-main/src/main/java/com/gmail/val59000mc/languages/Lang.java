...
public class Lang {
	...
	public static String TEAM_MESSAGE_NAME_TOO_LONG;
	public static String TEAM_MESSAGE_NAME_ILLEGAL_CHARACTERS;
	...
	private void loadLangConfig() {
		...
		// Team Inventories (titles have max length)
		...
		TEAM_MESSAGE_NAME_TOO_LONG = getString(lang, "team.message.name-too-long", "&cThat team name is too long!");
		TEAM_MESSAGE_NAME_ILLEGAL_CHARACTERS = getString(lang, "team.message.name-illegal-characters", "&cThat team name contains illegal characters!");
	...
	// ✅ HEX-enabled + safe trim
	private String getString(FileConfiguration lang, String path, String def, int maxLength) {
		String raw = lang.getString(path, def);
		String colored = TextUtil.toLegacy(raw);

		if (maxLength != -1 && colored != null && colored.length() > maxLength) {
			LOGGER.warning("The message " + path + " is too long, max length is " + maxLength + " characters");
			colored = TextUtil.safeTrimLegacy(colored, maxLength);
		}
	...
	private List<String> getStringList(FileConfiguration lang, String path, List<String> def) {
		...
		for (int i = 0; i < list.size(); i++) {
			list.set(i, TextUtil.toLegacy(list.get(i)));
		}
