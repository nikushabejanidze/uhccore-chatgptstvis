...
// ✅ ADDED
import com.gmail.val59000mc.customitems.ReviveItemCraftListener;
...
public class CraftsManager {
	...
	public static void loadCrafts() {
		...
		for (String name : craftsKeys) {
			...
			ItemStack craftItem;
			int limit;
			boolean defaultName, reviveItem, hidden;
			...
			try {
				...
				for (int i = 0; i < 3; i++) {
					...
					if (itemsInLine.length != 3) {
						throw new IllegalArgumentException(
							"Each line should be formatted like {item} {item} {item}, also make sure your items don't contain custom item data!"
						);
					}
					for (int j = 0; j < 3; j++) {
				...
				if (craftString.startsWith("{") && craftString.endsWith("}")) {
					...
					throw new IllegalArgumentException(
						"The craft result must be formatted according to the json item format (Use /iteminfo)."
					);
				}
				...
				// ✅ IMPORTANT: no craft.registerListener(...) here.
				// The revive behaviour is handled by the global Bukkit listener.
	...
	public static Craft getCraft(ItemStack result) {
		if (result != null && result.hasItemMeta() && result.getItemMeta().hasDisplayName()) {
			...
			for (Craft craft : getCrafts()) {
				if (craft.getCraft().hasItemMeta()
					&& craft.getCraft().getItemMeta().hasDisplayName()
					&& displayName.equals(craft.getCraft().getItemMeta().getDisplayName())) {
					return craft;
				}
			}
		}
	...
	public static Craft getCraftByDisplayName(String craftName) {
		for (Craft craft : getCrafts()) {
			if (craft.getDisplayItem().hasItemMeta()
				&& craft.getDisplayItem().getItemMeta().hasDisplayName()
				&& craft.getDisplayItem().getItemMeta().getDisplayName().equals(craftName)) {
				return craft;
			}
		}
		return null;
	...
	public static boolean isCraftItem(ItemStack item) {
		...
		if (item.hasItemMeta() && item.getItemMeta().hasDisplayName()) {
			...
			for (Craft craft : getCrafts()) {
				if (craft.getDisplayItem().hasItemMeta()
					&& craft.getDisplayItem().getItemMeta().hasDisplayName()
					&& name.equals(craft.getDisplayItem().getItemMeta().getDisplayName())
					&& item.getType().equals(craft.getCraft().getType())) return true;
			}
		}
	...
	public static boolean isCraftBookBackItem(ItemStack item) {
		...
		return item.getType().equals(UniversalMaterial.PUFFERFISH.getType())
			&& item.getItemMeta().getDisplayName().equals(Lang.ITEMS_CRAFT_BOOK_BACK);
	}
