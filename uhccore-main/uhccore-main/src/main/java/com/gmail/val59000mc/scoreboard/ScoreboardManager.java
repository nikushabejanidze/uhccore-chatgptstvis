...
public class ScoreboardManager {
	...
	// ✅ MUST MATCH GameManager call: new ScoreboardManager(scoreboardHandler, scoreboardLayout)
	...
	public String translatePlaceholders(String s, UhcPlayer uhcPlayer, Player bukkitPlayer) {
		...
		returnString = returnString
			...
			.replace("%needed%", cfg.get(MainConfig.MIN_PLAYERS_TO_START) + "");
		...
		if (returnString.contains("%alive%")) {
			if (gm.getScenarioManager().isEnabled(Scenario.SILENT_NIGHT)
		...
		if (returnString.contains("%playerSpectator%")) {
			long count = gm.getPlayerManager().getPlayersList().stream()
		...
		// ✅ REMOVED: old hard limit that wiped long lines
		// if (returnString.length() > 32) { warning; returnString = ""; }

		// ✅ APPLY HEX + & at the very end
		return TextUtil.toLegacy(returnString);
	}
